...
namespace VisProcess.Local.Controller
{
    ...
    [ApiController]
    public class ClientOrderLocalController : ControllerBase
    {
        private readonly IConfiguration _configuration;
        private readonly UnitWork _unitWork;
        private readonly ApplicationDbContext maildb;
        public ClientOrderLocalController(IConfiguration configuration, UnitWork unitWork, ApplicationDbContext db)
        {
        ...
        [HttpPost("SendMailForQueryJobs")]
        public async Task<object> SendMailForQueryJobs(int WFMId, string JobId, string StatusIdVal)
        {
            try
            {
                ...
                var QueryMailTriggering = _configuration.GetRequiredSection("QueryMailTriggering").Value;//ConfigurationManager.AppSettings["QueryMailTriggering"];
                ...
                if (WFMId != 0)
                {
                    var jobId = _unitWork.ProcessWorkFlowMaster.Where(x => x.Id == WFMId).FirstOrDefault().JobId;
                    pwtDetails = _unitWork.ProcessWorkFlowTran.Where(x => x.Wfmid == WFMId && (x.StatusId == 6 || x.StatusId == 8)).OrderByDescending(x => x.Id).FirstOrDefault();
                    jobOrderDetails = _unitWork.JobOrder.FirstOrDefault(x => x.Id == jobId);
                    jobQuery = _unitWork.JobQuery.Where(x => x.Wfmid == WFMId).OrderByDescending(x => x.Id).FirstOrDefault();
                }
                ...
                var getclientorderdetails = _unitWork.ClientOrder.FirstOrDefault(x => x.OrderId == jobOrderDetails.ClientOrderId);
                var emailIdAndName = _unitWork.CustomerContact.FirstOrDefault(x => x.CustomerId == getclientorderdetails.ClientId && x.ContactName == getclientorderdetails.SalesPersonName && x.IsDeleted == false);
                ...
                var mailNotify = await _unitWork.MailNotificationDetail.WhereAsync(x => x.CompareMailId == emailIdAndName.Id && x.IsDeleted == false && (x.Task == "All" || x.Task == "Query") && x.ClientId == getclientorderdetails.ClientId);
                ...
                {
                    Log addlog = new Log();
                    if (pwtDetails.StatusId == 6)
                    {
                        queryDetails = _unitWork.Status.FirstOrDefault(x => x.Id == pwtDetails.StatusId).Name;
                        body = "Hi " + emailIdAndName.ContactName + ",<br/><br/>" +
                          "VLead Team has raised a query regarding the order <b>" + getclientorderdetails.FileName + "</b>. Please find below the query and reply it in the portal.<br/><br/>" +
                          "We have provided the link to add your reply. Click the Query reply to proceed further.<br/><br/>" +
                          "<table border='1' cellpadding='5' cellspacing='0' style='border-collapse:collapse;'>" +
                          "<tr>" +
                          "<td>VLead Comments</td>" +
                          "<td>:</td>" +
                          "<td>" + jobQuery.Comments + "</td>" +
                          "<td><b><u><a href='" + QueryMailTriggering + "JId=" + JobId + "&StatusId=" + StatusIdVal + "&WFTId=" + pwtDetails.Id + "' target='_blank'>Query Reply</a></u></b></td>" +
                          "</tr>" +
                          "</table>" +
                          "<br/><br/>" +
                        ...
                        message.Subject = "Query Clarification Required - (" + getclientorderdetails.FileName + ")";
                        ...
                        addlog.Module = "Query Clarification Required";
                    }
                    else if (pwtDetails.StatusId == 8)
                    {
                        string estimatedTime = jobQuery.EstimatedTime != null ? jobQuery.EstimatedTime + " Mins" : "0 Mins";
                        decimal specialPrice = (decimal)(jobQuery.SpecialPrice != null ? jobQuery.SpecialPrice : 0);
                        queryDetails = _unitWork.Status.FirstOrDefault(x => x.Id == pwtDetails.StatusId).Name;
                        body = "Hi " + emailIdAndName.ContactName + ",<br/><br/>" +
                                "The order <b>" + getclientorderdetails.FileName + " </b> is considered as complex redraw due to its complexity and needs to priced specially." +
                                //"Below we mentioned link for login into your VIS account. <a href='" + QueryMailTriggering + "JId=" + JobId + "&StatusId=" + StatusIdVal + "&WFTId=" + pwtDetails.Id + "' target='_blank'>Click to Login VIS</a> " +
                                "We have provided the link to your VIS account login below. <br/><br/>" +
                                "<table border='1' cellpadding='5' cellspacing='0' style='border-collapse:collapse;'>" +
                                "<tr>" +
                                "<td>VLead Comments</td>" +
                                "<td>Estimated Time</td>" +
                                "<td>Price in $</td>" +
                                "<td>Click Below to approve</td>" +
                                "</tr>" +
                                "<tr>" +
                                "<td>" + jobQuery.Comments + "</td>" +
                                "<td>" + estimatedTime + "</td>" +
                        ...
                        message.Subject = "Query Requisition Approval Required - (" + getclientorderdetails.FileName + ")";
                        ...
                        addlog.Module = "Quote Requisition Required";
                    }
        ...
        [HttpPost("SendMailForCompletedJobs")]
        public async Task<object> SendMailForCompletedJobs(int JobId)
        {
                ...
                if (checkcustomerforbunchmail.BunchMail == false)
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            getclientorderdetailsJO = _unitWork.JobOrder.FirstOrDefault(x => x.ClientOrderId == jobOrderDetails.ClientOrderId); // changed
                            ...
                            if (CusOutputMailInfo.OutputType == "Email")
                            {
                                if (jobOrderDetails.FileName != null && pwtRecord.FileUploadPath != null)
                                {
                                    ...
                                    foreach (string fileName in fileEntries)
                                    {
                                        ...
                                        var name = System.IO.Path.GetExtension(fileName);
                        ...
                        Log addlog = new Log();
                        if ((jobstatusdes == "New") && (pwtDetails == null))
                        {
                            ...
                            using (var smtp = new SmtpClient(host, port))
                            {
                                ...
                                smtp.EnableSsl = Ssl;
                                //await smtp.SendMailAsync(message);////
                                //ApplicationDbContext maildb = new ApplicationDbContext();
                                var checkmailtriger = maildb.JobOrders.FirstOrDefault(x => x.Id == JobId);
                                if (checkmailtriger.IsMailTriggered == false || checkmailtriger.IsMailTriggered == null)
                                {
                                    await smtp.SendMailAsync(message);////
                                    checkmailtriger.IsMailTriggered = true;
                                    maildb.Entry(checkmailtriger).State = EntityState.Modified;
                                    maildb.SaveChanges();
                                }
                            }
